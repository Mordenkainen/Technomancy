        ...
        switch (itemDamage) {
            ...
            case 2:
                if (event.source instanceof EntityDamageSource) {
                    ...
                    int knockBack = 3;
                    hit.addVelocity(-MathHelper.sin((float) (Math.PI - hit.rotationYaw) * (float) Math.PI / 180.0F) * knockBack * 0.5F, 2.1D, MathHelper.cos((float) (Math.PI - hit.rotationYaw) * (float) Math.PI / 180.0F) * knockBack * 0.5F);
                }
                return;
        }
    }
    ...
    public void onTreasureDestroyed(World w, double posX, double posY, double posZ, EntityLivingBase entityLiving, int meta) {
        switch (meta) {
            case 0:
                ...
                int radius = 25;
                ArrayList<EntityLivingBase> l = (ArrayList<EntityLivingBase>) entityLiving.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, AxisAlignedBB.getBoundingBox(entityLiving.posX - radius, entityLiving.posY - radius, entityLiving.posZ - radius, entityLiving.posX + radius, entityLiving.posY + radius, entityLiving.posZ + radius));
            ...
            case 1:
                int range = 5;
                ArrayList<EntityLivingBase> l2 = (ArrayList<EntityLivingBase>) entityLiving.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, AxisAlignedBB.getBoundingBox(entityLiving.posX - range, entityLiving.posY - range, entityLiving.posZ - range, entityLiving.posX + range, entityLiving.posY + range, entityLiving.posZ + range));
            ...
            case 2:
                int range2 = 25;
                ArrayList<EntityLivingBase> l3 = (ArrayList<EntityLivingBase>) entityLiving.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, AxisAlignedBB.getBoundingBox(entityLiving.posX - range2, entityLiving.posY - range2, entityLiving.posZ - range2, entityLiving.posX + range2, entityLiving.posY + range2, entityLiving.posZ + range2));
                int knockBack = 3;
                for (EntityLivingBase hit : l3) {
                    hit.addVelocity(-MathHelper.sin((float) (Math.PI - hit.rotationYaw) * (float) Math.PI / 180.0F) * knockBack * 0.5F, 8D, MathHelper.cos((float) (Math.PI - hit.rotationYaw) * (float) Math.PI / 180.0F) * knockBack * 0.5F);
                }
        }
    }
