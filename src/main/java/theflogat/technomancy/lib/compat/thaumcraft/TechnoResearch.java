    ...
    private static void initThaumcraft() {
        ...
        if (TMConfig.itemMaterial) {
            ...
            if (TMConfig.dynEssentia || TMConfig.dynNode) {
                ...
                new ResearchItem("DYNAMO", "TECHNOMANCY", new AspectList().add(Aspect.MECHANISM, 5).add(Aspect.ENERGY, 5).add(Aspect.MAGIC, 5), 2, 2, 3, new ItemStack(iconBlock)).setPages(pages.toArray(new ResearchPage[pages.size()])).setParents(new String[] { "TECHNOBASICS" }).setSecondary().registerResearchItem();
            }
        }
        ...
        if (TMConfig.itemMaterial && TMConfig.wandCores) {
            ...
            if (TMConfig.scepter) {
                ...
                new ResearchItem("TECHNOTURGESCEPTER", "TECHNOMANCY", new AspectList().add(Aspect.TOOL, 5).add(Aspect.MAGIC, 5).add(Aspect.ENERGY, 5).add(Aspect.EXCHANGE, 5), -1, 3, 3, scepter3).setPages(new ResearchPage[] { new ResearchPage("techno.research_page.TECHNOTURGESCEPTER.1"), new ResearchPage((InfusionRecipe) recipes.get("TechnoturgeScepterRod")), new ResearchPage(scer.toArray(new IArcaneRecipe[0])) }).setParents(new String[] { "ROD_electric" }).setHidden()
                        .setItemTriggers(new ItemStack(TMItems.itemWandCores, 1)).registerResearchItem();
            }
        }
