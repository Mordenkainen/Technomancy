...
public final class BlockHelper {
	...
	public static List<ItemStack> breakBlock(World world, BlockPos pos, IBlockState state, int fortune, boolean doBreak, boolean silkTouch) {

		return breakBlock(world, null, pos, state, fortune, doBreak, silkTouch);
	}

	public static List<ItemStack> breakBlock(World world, EntityPlayer player, BlockPos pos, IBlockState state, int fortune, boolean doBreak, boolean silkTouch) {

		if (state.getBlockHardness(world, pos) == -1) {
			return new LinkedList<>();
		}
		List<ItemStack> stacks;
		if (silkTouch && state.getBlock().canSilkHarvest(world, pos, state, player)) {
			stacks = new LinkedList<>();
			stacks.add(createStackedBlock(state));
		} else {
			stacks = state.getBlock().getDrops(world, pos, state, fortune);
		}
		if (!doBreak) {
			return stacks;
		}
