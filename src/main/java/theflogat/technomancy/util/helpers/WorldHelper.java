...
public class WorldHelper {
    ...
    public static boolean isPlayerProbablyCollidingWithBlock(EntityPlayer player, int x, int y, int z) {
        return Math.floor(player.posX) - 1 > x && Math.ceil(player.posX) < x && Math.floor(player.posY) - 1 > y && Math.ceil(player.posY) < y && Math.floor(player.posZ) - 1 > z && Math.ceil(player.posZ) < z;
    }
}
